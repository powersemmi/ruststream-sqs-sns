...
Транспорт построен на официальном клиенте [`aws-sdk-sqs`](https://docs.rs/aws-sdk-sqs). Фича
`sns` добавляет публикацию в темы SNS через [`aws-sdk-sns`](https://docs.rs/aws-sdk-sns). По
умолчанию она выключена, и сервис, которому нужны только очереди, не компонует клиент SNS. Фича
`testing` даёт брокеру внутрипроцессный режим, и тест запускает рабочее приложение без сервера.
Фича `asyncapi` добавляет привязки `sqs` в документ, который создаёт фреймворк.
