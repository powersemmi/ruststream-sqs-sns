...
`create_if_missing` 需要创建的队列是标准队列；名字以 `.fifo` 结尾时，创建的是开启了基于内容的
...
队列用哪种写法命名都行。直接用一个名字挂载的注册（`#[subscriber("orders")]`，它和 Broker 之间
没有描述符）自己没有地方存放这份声明，于是声明由 Broker 接走，并把同一条重新驱动策略写到这个
名字打开的队列上。
...
这两项都是 FIFO 设置。为标准队列或者标准主题写出其中任何一项，都是一个发布错误
（`SqsError::NotFifo`）：调用方要的顺序在那里不会发生，而默默丢掉一个值是更差的答案。
`partition-key` 消息头不是对这个 Broker 的要求，因此标准队列照旧忽略它。
...
挂载点写出由谁送达。`.out_reply(policy)` 绑定它，调用之后的那些步骤（`.codec(..)` 和
`.transform(..)`）作用于这个位置。没写策略的挂载保留 `SqsPublish`，因此队列到队列的服务写
`b.include(handler);` 就够了。把同一个回复改发到主题，
是链上的一步：
...
回复什么也不调整，因为它没有调用点：`.out_reply(..)` 绑定的那个策略，就是它的全部答案。
...
往队列里放一个订单，让处理器的回复扇出去。`.out_reply(SnsPublish::default())` 就是扇出接线的
全部：
...
`MessageGroupId` 发出，投递时再带回来；标准目的地把它丢掉，因此标准队列既不转发它，也不返回它。
...
`SqsPublish` 定为自己的默认策略，因此 `.out_reply(Publish::default())`，以及带 `publish(..)` 的
