//! [`SqsPublisher`] (direct-to-queue) with its policy, and the per-message settings and builder
//! steps both of this crate's publishers share.
...
pub struct SqsPublisher {
    ...
    /// publishes and asks the broker where each publish went: to this queue, or to a topic of
    /// the same name.
...
/// `.out_reply(SqsPublish)` only ever restates the default. `SnsPublish` (the `sns` feature) is
