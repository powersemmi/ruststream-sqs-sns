...
impl AddressRouter {
    ...
    pub(crate) fn subscribe(
        ...
        self.state
            .lock()
            .expect("sqs test router mutex poisoned")
            .subscriptions
            .insert(
                id,
                Subscription {
    ...
    pub(crate) fn publish(
        ...
        {
            let mut state = self.state.lock().expect("sqs test router mutex poisoned");
            state
                .log
...
impl std::fmt::Debug for AddressRouter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let state = self.state.lock().expect("sqs test router mutex poisoned");
