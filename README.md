...
A handler that names an `SqsQueue` binds to the connected SQS broker, so leave the source to the mount site when the same handler is to run in process too.
...
#[subscriber(publish("orders-events"))]
async fn accept(order: &PlaceOrder) -> OrderPlaced {
    OrderPlaced { id: order.id }
}
